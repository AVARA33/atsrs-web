import { workableUrl } from './workable.mjs';
...
export function postingUrl(value, board, id, source) {
  if(source?.provider && source.provider!=='smartrecruiters') return allowedUrl(value,source);
  if (board === 'DOF') return workableUrl(value, id);
...
export function checkDetail(d, board, id, source) {
  ...
  if (!postingUrl(d.postingUrl, board, id, source) || !postingUrl(d.applyUrl, board, id, source)) return 'Unsupported application destination';
