-- Staging-first Stage 19 forward fix.
--
-- SQLSTATE 40001 denotes a serialization failure and may be retried below the
-- application transport. A stale workspace revision is a deterministic
-- application conflict, so it must use a non-retryable application SQLSTATE.
begin;
