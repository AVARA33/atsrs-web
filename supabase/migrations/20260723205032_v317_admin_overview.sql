...
create index if not exists atsrs_ai_usage_created_at_idx on public.atsrs_ai_usage (created_at desc);
...
insert into public.atsrs_admin_billing_config (id,purchased_credit_usd,baseline_spend_usd,baseline_recorded_at,updated_at)
values (true,5.00,0.0700,now(),now()) on conflict (id) do nothing;
...
begin
  v_is_admin := exists (select 1 from public.atsrs_admin_users au where au.user_id = auth.uid());
  if not v_is_admin then
    return query select false,null::bigint,null::bigint,null::numeric,null::numeric,null::numeric,null::bigint,null::timestamptz;
    return;
  end if;
  select c.purchased_credit_usd,c.baseline_spend_usd,c.baseline_recorded_at
    into v_credit,v_baseline_spend,v_baseline_at
    from public.atsrs_admin_billing_config c where c.id=true;
  select coalesce(sum(u.estimated_cost_usd),0),count(*),max(u.created_at)
    into v_usage_cost,v_tracked_scans,v_latest_usage
    from public.atsrs_ai_usage u where u.created_at>=v_baseline_at;
  return query
    select true,
      ...
      v_tracked_scans,
      greatest(v_baseline_at,coalesce(v_latest_usage,v_baseline_at));
end;
$$;
