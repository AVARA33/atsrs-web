...
alter table public.atsrs_notifications
  ...
  check (threshold_days = any (array[90, 60, 30, 7, 0]));
...
begin
  with cert_sources as (
    select
      ...
      wd.account_type,
      case
        when coalesce(wd.payload->>'value', '') ~ '^\s*\['
    ...
    select
      ...
      case
        ...
        when d.expiry_date - p_as_of <= 60 then 60
    ...
    select
      ...
      due.threshold_days,
      due.days_remaining,
      case due.threshold_days when 90 then 'notice' when 60 then 'notice' when 30 then 'warning' when 7 then 'urgent' else 'expired' end,
      case
        when due.days_remaining < 0 then 'ATSRS alert: ' || due.document_type || ' expired ' || abs(due.days_remaining) || case when abs(due.days_remaining) = 1 then ' day ago' else ' days ago' end
        when due.days_remaining = 0 then 'ATSRS expiry notice: ' || due.document_type || ' has reached its expiry date'
        when due.days_remaining <= 7 then 'ATSRS urgent: ' || due.document_type || ' expires in ' || due.days_remaining || case when due.days_remaining = 1 then ' day' else ' days' end
        when due.days_remaining <= 30 then 'ATSRS warning: ' || due.document_type || ' expires in ' || due.days_remaining || case when due.days_remaining = 1 then ' day' else ' days' end
        else 'ATSRS reminder: ' || due.document_type || ' expires in ' || due.days_remaining || ' days'
      end,
      case
        when due.days_remaining < 0 then 'Document: ' || due.document_type || '. Expiry date: ' || to_char(due.expiry_date, 'DD Mon YYYY') || '. Time overdue: ' || abs(due.days_remaining) || case when abs(due.days_remaining) = 1 then ' day.' else ' days.' end
        when due.days_remaining = 0 then 'Document: ' || due.document_type || '. Expiry date: ' || to_char(due.expiry_date, 'DD Mon YYYY') || '. This document has reached its expiry date today.'
        else 'Document: ' || due.document_type || '. Expiry date: ' || to_char(due.expiry_date, 'DD Mon YYYY') || '. Time remaining: ' || due.days_remaining || case when due.days_remaining = 1 then ' day.' else ' days.' end
      end
    from due
  ...
  where p.email_enabled
    and n.threshold_days in (90, 60, 30, 7, 0)
...
revoke all on function private.atsrs_queue_due_notifications(date)
