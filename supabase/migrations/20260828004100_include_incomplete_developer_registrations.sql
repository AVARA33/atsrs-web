-- The owner monitor includes started registrations even when Personal setup was
-- not completed, so abandoned onboarding remains visible for diagnosis.
...
begin
  ...
  v_is_admin := exists (
    select 1 from public.atsrs_admin_users as admin_user where admin_user.user_id = auth.uid()
  );
  ...
  select
    ...
    exists (
      select 1 from public.atsrs_workspaces as workspace
      where workspace.user_id = auth_user.id and workspace.account_type = 'personal'
    ),
    case
  ...
  left join public.atsrs_subscriptions as subscription on subscription.user_id = auth_user.id
  where auth_user.deleted_at is null
    ...
    and exists (select 1 from auth.identities as identity where identity.user_id = auth_user.id)
    and not coalesce(auth_user.raw_app_meta_data @> '{"atsrs_metrics_excluded": true}'::jsonb, false)
