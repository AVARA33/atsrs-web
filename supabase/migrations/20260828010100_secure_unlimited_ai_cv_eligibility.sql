-- Keep subscription/admin tables private while allowing the service-only CV
-- quota RPC to determine whether the caller receives unlimited generation.
create or replace function private.atsrs_ai_cv_is_unlimited(p_user_id uuid)
returns boolean
language sql
stable
security definer
...
revoke all on function private.atsrs_ai_cv_is_unlimited(uuid)
  from public, anon, authenticated, service_role;
grant execute on function private.atsrs_ai_cv_is_unlimited(uuid) to service_role;
...
begin
  ...
  v_unlimited := private.atsrs_ai_cv_is_unlimited(p_user_id);
  ...
  insert into public.atsrs_ai_cv_usage as usage (
    user_id, period_start, generation_count, updated_at
  ) values (
