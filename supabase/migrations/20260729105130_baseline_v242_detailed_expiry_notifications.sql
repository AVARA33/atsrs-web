-- Repository baseline for the already-live ATSRS V242 notification body.
-- This file is intended for clean/staging history reconstruction. It must be
-- marked applied, not replayed, when reconciling the current production DB.
begin;
  ...
  with cert_sources as (
    ...
    select
      ...
      case
        ...
        when due.days_remaining = 0 then 'ATSRS urgent: ' || due.document_type || ' expires today'
      ...
      case
        ...
        when due.days_remaining = 0 then 'Document: ' || due.document_type || '. Expiry date: ' || to_char(due.expiry_date, 'DD Mon YYYY') || '. This document expires today.'
