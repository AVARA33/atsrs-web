...
create index atsrs_hr_pending_balanced_candidate
on public.atsrs_job_ingestion_queue (
  specialty,
  ((job_id is not null)),
  (nullif(left(payload->>'releasedDate',10),'')) desc nulls last,
  discovered_at,
  board,
  external_id
)
...
language sql stable set search_path='' as $$
 with
 valid_lease as (
   select 1 from public.atsrs_job_ingestion_config c
 ...
 attempted_specialties as (
   ...
   from excluded e join public.atsrs_job_ingestion_queue q
 ...
 candidates as (
   select picked.* from categories c
   cross join lateral (
     select q.*
     from public.atsrs_job_ingestion_queue q
     join public.atsrs_job_sources s using(board)
     ...
     order by (q.job_id is not null),
       nullif(left(q.payload->>'releasedDate',10),'') desc nulls last,
       q.discovered_at,q.board,q.external_id
     limit 1
   ) picked
 )
 select q.* from candidates q
