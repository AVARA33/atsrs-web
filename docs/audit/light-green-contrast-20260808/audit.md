...
3. Live V441 Corporate action - healthy. The deployed control renders both foreground properties as white, the account avatar remains white on sage, and the ivory header controls retain their dark foreground.
4. Responsive/code regression - healthy. The correction changes only foreground rendering and preserves existing spacing, dimensions, touch targets, dark mode, and functionality.
