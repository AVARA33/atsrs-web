...
Frontend release: V390 (`js/server-data.js?v=390`,
`js/storage.js?v=390`, `js/dashboard.js?v=390`,
`js/workspace-switcher.js?v=390`)
...
- the V390 client hydrates deterministic UUIDv5 IDs before rendering and writes
  UUIDs on every later save;
- concurrent V390 tabs use `updated_at` optimistic concurrency. A stale tab is
...
6. Deploy the frontend commit containing V390.
7. Confirm the production HTML references `server-data.js?v=390`,
   `storage.js?v=390`, `dashboard.js?v=390`, and
   `workspace-switcher.js?v=390`. Verify the returned JavaScript contains
...
- Open two V390 tabs from the same account. Load the same workspace row in both.
...
2. Redeploy the previously recorded production frontend commit and its previous
   cache versions.
3. Hard refresh and close all V390 tabs.
...
deploy V390. If the frontend deployment fails, keep the compatibility flag
...
2. deploying/pushing V390 frontend code;
