...
(function(){
  ...
  function lockBuildBadge(){
    ...
    if(lines[0]&&lines[0].textContent!=='ATSRS V260')lines[0].textContent='ATSRS V260';
    if(lines[1]&&lines[1].textContent!=='Last Update: 21 Jul 2026')lines[1].textContent='Last Update: 21 Jul 2026';
  }
  function loadV241(){
    lockBuildBadge();
    loadAsset('link',{id:'atsrsNotificationsCss',rel:'stylesheet',href:'css/notifications.css?v=242'});
    loadAsset('script',{id:'atsrsNotificationsJs',src:'js/notifications.js?v=260'});
  }
  function appIsOpen(){
    var app=byId('app');
