/* ATSRS V331: session-aware refresh loading controller + notification assets. */
(function(){
  'use strict';
  var finished=false;
  var observer=null;
  ...
  function lockBuildBadge(){
    ...
    if(lines[0]&&lines[0].textContent!=='ATSRS V331')lines[0].textContent='ATSRS V331';
    if(lines[1]&&lines[1].textContent!=='Last Update: 24 Jul 2026')lines[1].textContent='Last Update: 24 Jul 2026';
  }
  function loadV241(){
    lockBuildBadge();
    loadAsset('link',{id:'atsrsNotificationsCss',rel:'stylesheet',href:'css/notifications.css?v=331'});
    loadAsset('script',{id:'atsrsNotificationsJs',src:'js/notifications.js?v=331'});
  }
  function appIsOpen(){
    var app=byId('app');
