...
(function(){
  ...
  function updateProfileSummary(p){
    ...
    var phone=p.phone||[(p.phoneCountryCode||''),(p.phoneLocal||'')].filter(Boolean).join(' ')||'Not provided',location=[p.address,p.city,p.country].map(function(value){return String(value||'').trim()}).filter(Boolean).join(', ')||'Not provided';
