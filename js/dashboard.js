...
(function(){
  ...
  function syncPhonePicker(select,forceInput){
    ...
    if(forceInput||document.activeElement!==select.__atsrsPicker.input)select.__atsrsPicker.input.value=code;
  }
  ...
  function profileSummaryDate(value,withTime){if(!value)return '—';var date=new Date(value);if(Number.isNaN(date.getTime()))return '—';var text=date.toLocaleDateString((window.atsrsLocaleCode?window.atsrsLocaleCode():'en-GB'),{day:'numeric',month:'short',year:'numeric'});return withTime?text+' · '+date.toLocaleTimeString((window.atsrsLocaleCode?window.atsrsLocaleCode():'en-GB'),{hour:'2-digit',minute:'2-digit'}):text;}
  function profileSummaryCity(p){var city=String(p&&p.city||'').trim();if(city)return city;var zone=String(p&&p.timezone||'');if(!zone.includes('/'))return '';return zone.split('/').pop().replace(/_/g,' ');}
  function updateProfileSummary(p){
    ...
    var phone=p.phone||[(p.phoneCountryCode||''),(p.phoneLocal||'')].filter(Boolean).join(' ')||'Not provided',location=[p.address,profileSummaryCity(p),p.country].map(function(value){return String(value||'').trim()}).filter(Boolean).join(', ')||'Not provided';
  ...
  var profileCalendarCursor=new Date();
  function profileStageSetText(id,value){var el=byId(id);if(el)el.textContent=String(value||'').trim()||'Not specified';}
  function profileStageDisplayDate(value){if(!value)return 'Not specified';var parts=String(value).match(/^(\d{4})-(\d{2})-(\d{2})/);var date=parts?new Date(Number(parts[1]),Number(parts[2])-1,Number(parts[3])):new Date(value);return Number.isNaN(date.getTime())?'Not specified':date.toLocaleDateString((window.atsrsLocaleCode?window.atsrsLocaleCode():'en-GB'),{day:'2-digit',month:'short',year:'numeric'});}
  ...
  function profileStageTimezoneLabel(value){var zone=value||'UTC',select=byId('profileTimezone'),label=select&&select.options[select.selectedIndex]&&select.options[select.selectedIndex].textContent||zone;try{return label+' · '+new Intl.DateTimeFormat((window.atsrsLocaleCode?window.atsrsLocaleCode():'en-GB'),{timeZone:zone,hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date());}catch(error){return label;}}
  function refreshProfileStageTimezone(){profileStageSetText('profileStageTimezone',profileStageTimezoneLabel(val('profileTimezone')||'UTC'));}
  function profileStageSetStatus(id,ok,positive,negative){var el=byId(id);if(!el)return;el.textContent='';var isVerifyAction=el.hasAttribute('data-profile-verify');if(ok||!isVerifyAction){var icon=document.createElement('i');icon.className=ok?'ph-fill ph-check-circle':'ph ph-x-circle';icon.setAttribute('aria-hidden','true');el.appendChild(icon);}el.appendChild(document.createTextNode(ok?positive:negative));el.classList.toggle('is-unverified',!ok);if(isVerifyAction)el.setAttribute('aria-label',(ok?'Verified ':'Verify ')+(el.getAttribute('data-profile-verify')==='whatsapp'?'WhatsApp number':'mobile phone'));}
  function renderProfileCalendar(){var month=byId('profileCalendarMonth'),days=byId('profileCalendarDays');if(!month||!days)return;var year=profileCalendarCursor.getFullYear(),monthIndex=profileCalendarCursor.getMonth(),first=new Date(year,monthIndex,1),mondayOffset=(first.getDay()+6)%7,today=new Date();month.textContent=first.toLocaleDateString((window.atsrsLocaleCode?window.atsrsLocaleCode():'en-GB'),{month:'long',year:'numeric'});days.textContent='';for(var index=0;index<42;index++){var date=new Date(year,monthIndex,index-mondayOffset+1),cell=document.createElement('span');cell.textContent=String(date.getDate());if(date.getMonth()!==monthIndex)cell.classList.add('is-outside');if(date.getFullYear()===today.getFullYear()&&date.getMonth()===today.getMonth()&&date.getDate()===today.getDate()){cell.classList.add('is-today');cell.setAttribute('aria-current','date');}days.appendChild(cell);}}
