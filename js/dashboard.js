...
(function(){
  ...
  function updateProfileStage(p){p=p||{};if(document.body&&document.body.classList.contains('personal-mode'))setText('accountTitle','Profile');var user=window.currentUser||{},phone=p.phone||[(p.phoneCountryCode||''),(p.phoneLocal||'')].filter(Boolean).join(' '),whatsapp=p.whatsapp||[(p.whatsappCountryCode||''),(p.whatsappLocal||'')].filter(Boolean).join(' '),confirmed=byId('profileStageAvailabilityConfirmed');
