...
(function(){
  ...
  function updateProfileStage(p){p=p||{};var user=window.currentUser||{},phone=p.phone||[(p.phoneCountryCode||''),(p.phoneLocal||'')].filter(Boolean).join(' '),whatsapp=p.whatsapp||[(p.whatsappCountryCode||''),(p.whatsappLocal||'')].filter(Boolean).join(' '),confirmed=byId('profileStageAvailabilityConfirmed');
