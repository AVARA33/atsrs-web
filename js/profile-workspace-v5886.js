(function(){
  ...
  function enterPersonal(focusId){
    ...
    editor.classList.remove('hidden');setEnabled(editor,true);byId('profilePage').classList.add('profile-inline-editing');var actions=byId('profileInlineActions');if(actions)actions.hidden=false;
  ...
  function stackPersonalEditor(editor){
    var read=byId('profilePersonalReadView');if(!read||!editor||read.parentNode!==editor.parentNode)return;
    var stack=document.createElement('div');stack.className='profile-personal-grid-stack';read.parentNode.insertBefore(stack,read);stack.appendChild(read);stack.appendChild(editor);
  }
  ...
  function init(){
    ...
    state.ready=true;editor.appendChild(grid);buildPersonalEditor(editor,grid);stackPersonalEditor(editor);bindPersonalActions();setEnabled(editor,false);
