(function(){
  ...
  function enterPersonal(focusId){
    ...
    var stagedPhone=byId('profileStagePhone')&&byId('profileStagePhone').textContent||'',stagedWhatsapp=byId('profileStageWhatsapp')&&byId('profileStageWhatsapp').textContent||'';if(window.loadProfile)window.loadProfile();var nationality=byId('profileInlineNationality'),country=byId('profileCountry'),birthDisplay=byId('profileInlineBirthDateDisplay'),phoneDisplay=byId('profileInlinePhoneDisplay'),whatsappDisplay=byId('profileInlineWhatsappDisplay');if(nationality)nationality.value=country&&country.value||'';if(birthDisplay)birthDisplay.value=byId('profileStageBirthDate')&&byId('profileStageBirthDate').textContent||'';if(phoneDisplay)phoneDisplay.value=stagedPhone;if(whatsappDisplay)whatsappDisplay.value=stagedWhatsapp;state.personalEditing=true;state.initialDraft=draft();
