(function(){
  ...
  function buildPersonalEditor(editor,source){
    ...
    field(0,'profileName');field(1,'profileSurname');var birthDisplay=document.createElement('input'),birthPicker=document.createElement('input'),birthHost=document.createElement('div');birthDisplay.id='profileInlineBirthDateDisplay';birthDisplay.readOnly=true;birthDisplay.setAttribute('aria-label','Date of birth');birthPicker.id='profileInlineBirthPicker';birthPicker.type='date';birthPicker.setAttribute('aria-label','Choose date of birth');birthHost.className='profile-birth-picker-host';birthHost.appendChild(birthPicker);document.body.appendChild(birthHost);var birthStrong=boxes[2]&&boxes[2].querySelector('strong');if(birthStrong){birthStrong.replaceWith(birthDisplay);boxes[2].tabIndex=0;boxes[2].setAttribute('role','button');boxes[2].addEventListener('click',function(){birthPicker.click()});boxes[2].addEventListener('keydown',function(event){if(event.key==='Enter'||event.key===' '){event.preventDefault();birthPicker.click()}})}birthPicker.addEventListener('change',function(){if(!birthPicker.value)return;var parts=birthPicker.value.split('-'),date=new Date(Number(parts[0]),Number(parts[1])-1,Number(parts[2]));birthDisplay.value=new Intl.DateTimeFormat('en-GB',{day:'2-digit',month:'short',year:'numeric'}).format(date)});
    ...
    field(4,'profileCompany');field(5,'profilePosition');field(6,'profileAddress');field(7,'profileZipCode');var countryDisplay=document.createElement('div'),countryStrong=boxes[8]&&boxes[8].querySelector('strong');countryDisplay.id='inlineCountryValue';countryDisplay.contentEditable='true';countryDisplay.setAttribute('role','textbox');if(countryStrong)countryStrong.replaceWith(countryDisplay);
