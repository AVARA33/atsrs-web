(function(){
  ...
  function enterPersonal(focusId){
    ...
    if(window.loadProfile)window.loadProfile();var nationality=byId('profileInlineNationality'),country=byId('profileCountry'),birthDisplay=byId('profileInlineBirthDateDisplay');if(nationality)nationality.value=country&&country.value||'';if(birthDisplay)birthDisplay.value=byId('profileStageBirthDate')&&byId('profileStageBirthDate').textContent||'';state.personalEditing=true;state.initialDraft=draft();
