(function(){
  ...
  function draft(){var root=byId('profilePersonalReadView');if(!root)return '';return Array.prototype.slice.call(root.querySelectorAll('.profile-inline-control')).map(function(el){return el.id+'='+String(el.type==='checkbox'?el.checked:el.value)}).join('&')}
  function setPersonalControls(enabled){var root=byId('profilePersonalReadView');if(!root)return;Array.prototype.forEach.call(root.querySelectorAll('.profile-inline-control'),function(el){el.disabled=!enabled})}
  ...
  function enterPersonal(focusId){
    ...
    activate('personal',false);var read=byId('profilePersonalReadView');if(!read)return;
    var nationality=byId('profileInlineNationality'),nationalityDisplay=byId('profileStageNationality'),birthCountrySource=byId('profileBirthCountry'),birthCountry=byId('profileInlineBirthCountry'),birthCountryDisplay=byId('profileStageBirthCountry'),citySource=byId('profileCity'),city=byId('profileInlineCity'),cityDisplay=byId('profileStageCity'),country=byId('profileCountry'),inlineCountry=byId('profileInlineCountryResidence'),countryDisplay=byId('profileStageCountry'),company=byId('profileCompany'),inlineCompany=byId('profileInlineCompany'),companyDisplay=byId('profileStageCompany'),timezoneSource=byId('profileTimezone'),timezone=byId('profileInlineTimezone'),timezoneDisplay=byId('profileStageTimezone'),birthDisplay=byId('profileInlineBirthDateDisplay'),birthPicker=byId('profileInlineBirthPicker'),birth=byId('profileBirthDate');if(nationality)nationality.value=nationalityDisplay&&nationalityDisplay.textContent||nationality.value||country&&country.value||'';if(nationalityDisplay)nationalityDisplay.textContent=selectedText(nationality);if(birthCountry)birthCountry.value=birthCountryDisplay&&birthCountryDisplay.textContent||birthCountrySource&&birthCountrySource.value||'';if(birthCountryDisplay)birthCountryDisplay.textContent=selectedText(birthCountry);populateCitySelect(city,birthCountry&&birthCountry.value||'',cityDisplay&&cityDisplay.textContent||citySource&&citySource.value||'');if(cityDisplay)cityDisplay.textContent=selectedText(city);if(inlineCountry)inlineCountry.value=countryDisplay&&countryDisplay.textContent||country&&country.value||'';if(countryDisplay)countryDisplay.textContent=selectedText(inlineCountry);if(inlineCompany){var companyValue=companyDisplay&&companyDisplay.textContent||company&&company.value||'';if(companyValue&&!Array.prototype.some.call(inlineCompany.options,function(option){return option.value===companyValue})){var companyOption=document.createElement('option');companyOption.value=companyOption.textContent=companyValue;inlineCompany.appendChild(companyOption)}inlineCompany.value=companyValue}if(companyDisplay)companyDisplay.textContent=selectedText(inlineCompany);if(timezone)timezone.value=timezoneSource&&timezoneSource.value||'UTC';if(timezoneDisplay)timezoneDisplay.textContent=timezoneEditorText(timezone);if(birthDisplay)birthDisplay.value=byId('profileStageBirthDate')&&byId('profileStageBirthDate').textContent||'';if(birthPicker)birthPicker.value=birth&&birth.value||'';state.initialBirthDisplay=birthDisplay&&birthDisplay.value||'';state.initialBirthIso=birthPicker&&birthPicker.value||'';state.personalEditing=true;setPersonalControls(true);state.initialDraft=draft();
    byId('profilePage').classList.add('profile-inline-editing');var actions=byId('profileInlineActions');if(actions)actions.hidden=false;
    var target=focusId?byId(focusId):null;if(target)setTimeout(function(){target.focus({preventScroll:true})},0);window.scrollTo(savedX,savedY);
  }
  function finishPersonal(){var savedX=window.scrollX,savedY=window.scrollY,summaryEdit=byId('profileSummaryEditBtn'),actions=byId('profileInlineActions');state.personalEditing=false;state.initialDraft='';state.initialBirthDisplay='';state.initialBirthIso='';setPersonalControls(false);if(summaryEdit){summaryEdit.hidden=false;summaryEdit.focus({preventScroll:true})}if(actions)actions.hidden=true;byId('profilePage').classList.remove('profile-inline-editing');window.scrollTo(savedX,savedY)}
  ...
  function buildPersonalEditor(editor,source){
    ...
    var read=byId('profilePersonalReadView'),layout=read;layout.classList.add('profile-inline-grid');
    var boxes=layout.querySelectorAll('.profile-information-field');
    function prepare(control,box,kind){if(!control||!box)return control;var shell=control.closest('.atsrs-field-shell');if(shell) shell.replaceWith(control);control.setAttribute('data-atsrs-no-field-shell','true');control.classList.add('profile-inline-control');box.classList.add(kind);return control}
    function field(index,id){var control=prepare(byId(id),boxes[index],'profile-inline-input-controlled'),strong=boxes[index]&&boxes[index].querySelector('strong');if(strong&&control)strong.after(control);return control}
    field(0,'profileName');field(1,'profileSurname');var birthDisplay=document.createElement('input'),birthPicker=document.createElement('input'),birthHost=document.createElement('div'),birthTrigger=document.createElement('button');birthDisplay.id='profileInlineBirthDateDisplay';birthDisplay.readOnly=true;birthDisplay.setAttribute('aria-label','Date of birth');prepare(birthDisplay,boxes[2],'profile-inline-input-controlled');birthPicker.id='profileInlineBirthPicker';birthPicker.type='date';birthPicker.setAttribute('aria-label','Choose date of birth');birthPicker.dataset.atsrsDatePosition='anchor-above-right';birthPicker.dataset.atsrsDateAnchorId='profileInlineBirthDateDisplay';birthHost.className='profile-birth-picker-host';birthHost.appendChild(birthPicker);document.body.appendChild(birthHost);function openBirthPicker(){birthPicker.click()}birthDisplay.addEventListener('click',openBirthPicker);birthTrigger.type='button';birthTrigger.className='profile-birth-trigger';birthTrigger.setAttribute('aria-label','Open date of birth calendar');birthTrigger.addEventListener('click',openBirthPicker);var birthStrong=boxes[2]&&boxes[2].querySelector('strong');if(birthStrong){birthStrong.after(birthDisplay);boxes[2].appendChild(birthTrigger)}birthPicker.addEventListener('change',function(){if(!birthPicker.value)return;var parts=birthPicker.value.split('-'),date=new Date(Number(parts[0]),Number(parts[1])-1,Number(parts[2]));birthDisplay.value=new Intl.DateTimeFormat('en-GB',{day:'2-digit',month:'short',year:'numeric'}).format(date)});
    ...
    function customSelect(index,select,displayId,onInput){var box=boxes[index],strong=box&&box.querySelector('strong');if(!box||!strong||!select)return null;strong.dataset.inlineDisplayId=displayId;prepare(select,box,'profile-inline-select-controlled');strong.after(select);box.querySelectorAll('i').forEach(function(icon){icon.remove()});box.classList.add('profile-nationality-select');select.addEventListener('input',function(){strong.textContent=selectedText(select);if(onInput)onInput(select,strong)});return select}
  ...
  function closeSecurityContactEditor(){var modal=byId('profileSecurityContactEditor');if(modal)modal.remove();document.body.classList.remove('profile-security-contact-open')}
  function openSecurityContactEditor(kind,trigger){
    closeSecurityContactEditor();
    var whatsapp=kind==='whatsapp',sourceCode=byId(whatsapp?'profileWhatsappCountryCode':'profilePhoneCountryCode'),sourceLocal=byId(whatsapp?'profileWhatsappLocal':'profilePhoneLocal');
    if(!sourceCode||!sourceLocal)return;
    var modal=document.createElement('div');modal.id='profileSecurityContactEditor';modal.className='profile-security-contact-editor';
    modal.innerHTML='<button type="button" class="profile-security-contact-backdrop" aria-label="Close"></button><section class="profile-security-contact-dialog" role="dialog" aria-modal="true" aria-labelledby="profileSecurityContactTitle"><header><div><span>SECURITY CONTACT</span><h3 id="profileSecurityContactTitle">Edit '+(whatsapp?'WhatsApp number':'mobile phone')+'</h3></div><button type="button" class="profile-security-contact-close" aria-label="Close">&times;</button></header><p>The number stays private and must be saved before verification.</p><div class="profile-security-contact-fields"><label><span>Country calling code</span><select id="profileSecurityContactCode"></select></label><label><span>'+(whatsapp?'WhatsApp number':'Mobile number')+'</span><input id="profileSecurityContactLocal" inputmode="tel" autocomplete="tel-national" placeholder="Enter number"></label></div><p id="profileSecurityContactStatus" class="profile-security-contact-status" role="status"></p><footer><button type="button" class="secondary" data-security-contact-cancel>Cancel</button><button type="button" data-security-contact-save>Save number</button></footer></section>';
    document.body.appendChild(modal);document.body.classList.add('profile-security-contact-open');
    var code=byId('profileSecurityContactCode'),local=byId('profileSecurityContactLocal'),status=byId('profileSecurityContactStatus'),save=modal.querySelector('[data-security-contact-save]'),originalCode=sourceCode.value,originalLocal=sourceLocal.value;
    Array.prototype.forEach.call(sourceCode.options,function(option){code.appendChild(option.cloneNode(true))});code.value=originalCode||'+994';local.value=originalLocal||'';
    function close(){closeSecurityContactEditor();if(trigger&&typeof trigger.focus==='function')trigger.focus({preventScroll:true})}
    modal.querySelector('.profile-security-contact-backdrop').addEventListener('click',close);modal.querySelector('.profile-security-contact-close').addEventListener('click',close);modal.querySelector('[data-security-contact-cancel]').addEventListener('click',close);
    local.addEventListener('input',function(){local.value=local.value.replace(/\D/g,'').slice(0,15)});
    modal.addEventListener('keydown',function(event){if(event.key==='Escape')close();if(event.key==='Enter'&&event.target===local)save.click()});
    save.addEventListener('click',async function(){var digits=local.value.replace(/\D/g,'');if(digits.length<7){status.textContent='Enter a valid phone number.';local.focus();return}save.disabled=true;status.textContent='Saving...';sourceCode.value=code.value;sourceLocal.value=digits;sourceCode.dispatchEvent(new Event('change',{bubbles:true}));sourceLocal.dispatchEvent(new Event('input',{bubbles:true}));var ok=window.saveProfile?await window.saveProfile({contactChanged:whatsapp?'whatsapp':'mobile'}):false;if(ok){activate('security',false);close()}else{sourceCode.value=originalCode;sourceLocal.value=originalLocal;sourceCode.dispatchEvent(new Event('change',{bubbles:true}));sourceLocal.dispatchEvent(new Event('input',{bubbles:true}));status.textContent='The number could not be saved. Check the connection and try again.';save.disabled=false}});
    setTimeout(function(){local.focus()},0);
  }
  function init(){
    ...
    document.querySelectorAll('[data-profile-stage-edit]').forEach(function(button){button.addEventListener('click',function(){openSecurityContactEditor(button.dataset.profileStageEdit==='whatsapp'?'whatsapp':'mobile',button)})});
    return true;
  }
