(function(){
  ...
  function buildPersonalEditor(editor,source){
    ...
    field(0,'profileName');field(1,'profileSurname');var birthDisplay=document.createElement('input'),birthPicker=document.createElement('input');birthDisplay.id='profileInlineBirthDateDisplay';birthDisplay.setAttribute('aria-label','Date of birth');birthPicker.id='profileInlineBirthPicker';birthPicker.type='date';birthPicker.setAttribute('aria-label','Choose date of birth');var birthStrong=boxes[2]&&boxes[2].querySelector('strong');if(birthStrong){birthStrong.replaceWith(birthDisplay);boxes[2].appendChild(birthPicker)}birthPicker.addEventListener('change',function(){if(!birthPicker.value)return;var parts=birthPicker.value.split('-'),date=new Date(Number(parts[0]),Number(parts[1])-1,Number(parts[2]));birthDisplay.value=new Intl.DateTimeFormat('en-GB',{day:'2-digit',month:'short',year:'numeric'}).format(date)});
