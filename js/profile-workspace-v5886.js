(function(){
  ...
  function closeSecurityContactEditor(focusTrigger){var editor=byId('profileSecurityContactEditor'),card=editor&&editor.closest('.profile-security-contact-card');if(editor)editor.remove();if(card)card.classList.remove('is-editing');if(focusTrigger&&typeof focusTrigger.focus==='function')focusTrigger.focus({preventScroll:true})}
  function openSecurityContactEditor(kind,trigger){
    closeSecurityContactEditor();
    var whatsapp=kind==='whatsapp',card=trigger&&trigger.closest('.profile-security-contact-card'),sourceCode=byId(whatsapp?'profileWhatsappCountryCode':'profilePhoneCountryCode'),sourceLocal=byId(whatsapp?'profileWhatsappLocal':'profilePhoneLocal');
    if(!card||!sourceCode||!sourceLocal)return;
    var editor=document.createElement('div');editor.id='profileSecurityContactEditor';editor.className='profile-security-contact-editor';
    editor.innerHTML='<div class="profile-security-contact-fields"><label><span>Country code</span><select id="profileSecurityContactCode" aria-label="Country calling code"></select></label><label><span>'+(whatsapp?'WhatsApp number':'Mobile number')+'</span><input id="profileSecurityContactLocal" inputmode="tel" autocomplete="tel-national" placeholder="Enter number"></label></div><div class="profile-security-contact-footer"><p id="profileSecurityContactStatus" class="profile-security-contact-status" role="status"></p><div><button type="button" class="secondary" data-security-contact-cancel>Cancel</button><button type="button" data-security-contact-save>Save</button></div></div>';
    card.appendChild(editor);card.classList.add('is-editing');
    var code=byId('profileSecurityContactCode'),local=byId('profileSecurityContactLocal'),status=byId('profileSecurityContactStatus'),save=editor.querySelector('[data-security-contact-save]'),originalCode=sourceCode.value,originalLocal=sourceLocal.value;
    ...
    function close(){closeSecurityContactEditor(trigger)}
    editor.querySelector('[data-security-contact-cancel]').addEventListener('click',close);
    local.addEventListener('input',function(){local.value=local.value.replace(/\D/g,'').slice(0,15)});
    editor.addEventListener('keydown',function(event){if(event.key==='Escape')close();if(event.key==='Enter'&&event.target===local)save.click()});
