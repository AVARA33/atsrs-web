(function(){
  ...
  var plans=[
    {key:'bronze',accent:'#a96d36',name:'BRONZE',status:'RECOMMENDED',title:'Active Job Seekers',price:'$20',usd:20,azn:34,period:'/ month',note:'Monthly billing',features:['1000 MB Secure Storage','200 Tracked Documents','Unlimited Email + 150 WhatsApp expiry alerts / month','15 AI document scans + 3 AI CV generations / month','Full JobSearch vacancy catalogue','Immediate access to newest vacancies','Recruiter details and verified contact routes','Direct Apply and original source links','Recruiter and official company directories','24-hour recipient-specific profile sharing']},
    {key:'silver',accent:'#8a9baa',name:'SILVER',status:'COMING SOON',title:'Frequent Career Activity',price:'$40',usd:40,azn:68,period:'/ month',note:'Planned price',features:['2 GB Secure Storage','700 Tracked Documents','Unlimited Email + 300 WhatsApp expiry alerts / month','50 AI document scans + 5 AI CV generations / month','Full JobSearch vacancy catalogue','Immediate access to newest vacancies','Recruiter details and verified contact routes','Direct Apply and original source links','Recruiter and official company directories','24-hour recipient-specific profile sharing']},
    {key:'gold',accent:'#c99b2d',name:'GOLD',status:'COMING SOON',title:'Intensive Career Management',price:'$70',usd:70,azn:119,period:'/ month',note:'Planned price',features:['5 GB Secure Storage','1,500 Tracked Documents','Unlimited Email + 750 WhatsApp expiry alerts / month','150 AI document scans + 7 AI CV generations / month','Full JobSearch vacancy catalogue','Immediate access to newest vacancies','Recruiter details and verified contact routes','Direct Apply and original source links','Recruiter and official company directories','24-hour recipient-specific profile sharing']},
    {key:'titan',accent:'#8d79b8',name:'TITAN',status:'COMING SOON',title:'Maximum Personal Capacity',price:'$120',usd:120,azn:204,period:'/ month',note:'Planned price',features:['10 GB Secure Storage','2,000 Tracked Documents','Unlimited Email and WhatsApp expiry alerts','500 AI document scans + 10 AI CV generations / month','Full JobSearch vacancy catalogue','Immediate access to newest vacancies','Recruiter details and verified contact routes','Direct Apply and original source links','Recruiter and official company directories','24-hour recipient-specific profile sharing']}
  ];
  var planCarousel=root.querySelector('.atlas-plan-carousel'),planControls=root.querySelector('.atlas-plan-controls'),planIndex=0,planTimer=0,planTransitionTimer=0,planVibrationTimer=0,planVibrationEndTimer=0,planCurrency=null;
  function planCardMarkup(plan,entering){var price=planCurrency?planCurrency.convert(plan.usd,plan.azn):plan.price;return '<article class="atlas-plan-card is-'+plan.key+(entering?' is-entering':'')+'"><header><span>'+plan.status+'</span><p>'+plan.name+'</p></header><h3>'+plan.title+'</h3><div class="atlas-plan-price"><strong>'+price+'</strong><span>'+plan.period+'</span><small>'+plan.note+'</small></div><ul>'+plan.features.map(function(feature){return '<li>'+feature+'</li>'}).join('')+'</ul></article>'}
  ...
  function showPlan(index,instant){
    ...
    var link=root.querySelector('.atlas-plan-link');if(link)link.href='/?route=resource&resource=plans#'+plan.key;
  }
  ...
  var allPlansLink=root.querySelector('.atlas-plan-link');if(allPlansLink)allPlansLink.addEventListener('click',function(event){if(!window.atsrsOpenAccountResource)return;event.preventDefault();window.atsrsOpenAccountResource('plans')});
  var pricingShowcase=root.querySelector('.atlas-pricing-showcase');if(pricingShowcase){pricingShowcase.addEventListener('pointerenter',function(){clearInterval(planTimer)});pricingShowcase.addEventListener('pointerleave',startPlans);pricingShowcase.addEventListener('focusin',function(){clearInterval(planTimer)});pricingShowcase.addEventListener('focusout',startPlans);if(window.atsrsPricingCurrency&&window.atsrsPricingCurrency.mountPicker)window.atsrsPricingCurrency.mountPicker(pricingShowcase,function(ctx){planCurrency=ctx;showPlan(planIndex,true)})}
