...
(function(){
  ...
  function ensurePersonalToolsGrid(stats){
    ...
    storage.innerHTML='<div class="dashboard-personal-tool-head"><div><h2 id="dashboardStorageTitle">Storage Usage</h2><p id="dashboardStoragePlan" class="sub">Personal plan · secure server storage</p></div><button id="dashboardManageStorage" type="button" class="secondary">Manage Storage</button></div><div class="dashboard-storage-visual"><div class="dashboard-storage-chart" role="img" aria-label="Personal storage usage"><canvas id="dashboardStorageChart" width="112" height="112" aria-hidden="true"></canvas><strong id="dashboardStoragePercent">—</strong></div><div class="dashboard-storage-copy"><div class="dashboard-storage-summary"><strong id="dashboardStorageUsed">—</strong><span id="dashboardStorageDetail" aria-live="polite"> stored</span></div><p>Secure server encrypted archival storage is active.</p></div></div>';
    grid.appendChild(quick);grid.appendChild(storage);stats.insertAdjacentElement('afterend',grid);
    byId('dashboardManageStorage').addEventListener('click',function(){
      if(typeof window.atsrsOpenAccountResource==='function')window.atsrsOpenAccountResource('plans');
    });
