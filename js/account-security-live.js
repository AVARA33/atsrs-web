...
(function(){
  ...
  async function submitPasswordChange(){
    var p1=byId('atsrsNewPassword').value,p2=byId('atsrsConfirmPassword').value,button=byId('atsrsSubmitPassword');if(p1.length<10){message('Use at least 10 characters.','error');return}if(p1!==p2){message('Passwords do not match.','error');return}
    busy(button,true,'Updating...');try{var result=await getClient().auth.updateUser({password:p1});if(result.error)throw result.error;message('Password updated successfully.','ok');byId('atsrsNewPassword').value='';byId('atsrsConfirmPassword').value=''}catch(e){message(e.message||'Password could not be updated.','error')}finally{busy(button,false)}
  }
  ...
  function bind(){
    ...
    var email=byId('primaryEmailBtn');if(email){email.removeAttribute('onclick');email.onclick=openEmailChange}
    var password=byId('changePasswordBtn');if(password){password.removeAttribute('onclick');password.onclick=openPasswordChange}
