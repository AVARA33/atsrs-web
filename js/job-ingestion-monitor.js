...
(function(){
 ...
 window.atsrsRefreshJobMonitor=async function(force){
  ...
  [['Shared API balance · last verified',b?'$'+Number(b.amount_usd).toFixed(2):'Unavailable'],['HR daily limit',money(d.daily_limit)],['HR today · used',money(d.today_cost)],['HR today · remaining',money(d.daily_remaining)],['HR today · reserved',money(d.today_reserved)],['HR this month · calculated',money(d.month_cost)],['HR monthly limit',money(d.monthly_limit)],['Month reserved · unresolved',money(d.unresolved_reserve)],['AI Scan cost','Not connected']].forEach(function(p,index){var box=cell(metrics,'div','');cell(box,'span',p[0]);cell(box,'strong',p[1]);if(index===0){var balanceRefresh=cell(box,'button','');balanceRefresh.type='button';balanceRefresh.className='job-monitor-balance-refresh';balanceRefresh.innerHTML='<i class="ph ph-arrow-clockwise" aria-hidden="true"></i>';var balanceRefreshLabel=(window.atsrsLocaleCode&&String(window.atsrsLocaleCode()).toLowerCase().indexOf('az')===0)?'API balansını yenilə':'Refresh API balance';balanceRefresh.setAttribute('aria-label',balanceRefreshLabel);balanceRefresh.title=balanceRefreshLabel;balanceRefresh.onclick=function(){window.open('https://platform.openai.com/settings/organization/billing/overview','_blank','noopener,noreferrer');};}});
