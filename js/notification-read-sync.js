...
(function(){
 'use strict';
 var uid=null,reads=new Set(),generation=0,pending=null,lastSync=0,bound=null,checked=new Set(),rerun=false,syncError=false;
 ...
 function reset(){uid=null;reads=new Set();checked=new Set();syncError=false;generation++;lastSync=0;changed()}
 ...
 async function refresh(){
  ...
  if(context.generation===generation){reads=next;ids.forEach(function(id){checked.add(id)});syncError=false;changed()}
 }
 function sync(force){
  if(pending){if(force)rerun=true;return pending;}
  if(!force&&Date.now()-lastSync<30000)return Promise.resolve();
  lastSync=Date.now();pending=refresh().catch(function(){syncError=true;changed()}).finally(function(){pending=null;if(rerun){rerun=false;return sync(true)}});return pending;
 }
 async function mark(rows){
  ...
  if(context.generation===generation){ids.forEach(function(id){reads.add(id);checked.add(id)});changed()}
 }
 window.atsrsNotificationReads={has:function(id){return reads.has(id)},ready:function(rows){return !!uid&&idsOf(rows).every(function(id){return checked.has(id)})},failed:function(){return syncError},sync:sync,mark:mark};
