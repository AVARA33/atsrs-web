...
function syncPersonalHeadingHierarchy(page){
  const personal=isPersonalMode(),legal=personal&&(page==="privacy"||page==="dataRights"||page==="billing");
...
function hydrateLegalFrame(page){
  if(page!=="privacy"&&page!=="dataRights"&&page!=="billing")return;
...
restoreCurrentPage=function(options){let params=new URLSearchParams(location.search),route=params.get('route'),developerRequested=route==='developer',stored=localStorage.getItem('atsrs_current_page')||'intro',resourceLink=document.querySelector('#sidebarQuickLinks [data-resource="'+String(params.get('resource')||'plans').replace(/[^a-z-]/g,'')+'"]');let map={intro:navIntro,privacy:navPrivacy,dataRights:navPrivacy,dashboard:navDashboard,candidates:navCandidates,personnel:navPersonnel,projects:navProjects,certificates:navCertificates,refs:navRefs,compliance:navCompliance,security:navCompliance,reports:navReports,profile:navProfile,jobs:navJobs,employers:navEmployers,recruiters:navRecruiters,developer:navDeveloper,resource:resourceLink};let page=map[route]?route:(map[stored]?stored:'intro');window.__atsrsRequestedDeveloperRoute=developerRequested;showPage(page,map[page],options||{replace:true});if(developerRequested&&window.__atsrsDeveloperAccess!==true){let routeUrl=new URL(location.href);routeUrl.searchParams.set('route','developer');history.replaceState({},'',routeUrl.pathname+routeUrl.search+routeUrl.hash);}if(page==='profile'&&params.get('tab')==='sharing')setTimeout(function(){showAccountTab('sharing')},0);if(page==='resource'&&window.atsrsSyncAccountResource)setTimeout(window.atsrsSyncAccountResource,0);}
if(!window.__atsrsPageHistoryBound){window.__atsrsPageHistoryBound=true;window.addEventListener('popstate',function(){restoreCurrentPage({replace:true});if(new URLSearchParams(location.search).get('route')==='jobs')window.dispatchEvent(new CustomEvent('atsrs:jobs-nav'))})}
const V27_TEXT={
  ...
  function restoreSession(){
    ...
    async function handleSignUp(user,event){
      ...
      try{
        ...
        if(created.duplicate){
          state=await readWorkspaceState(user);
          if(hasWorkspace(state,pendingMode)){
            await openExistingWorkspace(user,pendingMode,state);
            return;
          }
          returnToLogin(user,workspaceServiceMessage(new Error('The existing account could not be opened.')));
        }
      }catch(error){
    ...
    window.atsrsCompleteRecoveredSignup=async function(user,mode){
      ...
      try{
        ...
        if(created.duplicate){
          state=await readWorkspaceState(user);
          if(hasWorkspace(state,mode)) return await openExistingWorkspace(user,mode,state);
          returnToLogin(user,workspaceServiceMessage(new Error('The existing account could not be opened.')));
          return false;
        }
        return true;
...
(function(){
  ...
  window.atsrsHandleAccountTypeChoice=async function(mode){
    ...
    if(mode==='company' && ctx==='signup'){
      setLoginMsg('Corporate Account is in development. Personal registration is available now.');
      return;
    }
    applyMode(mode);
    if(ctx==='signup'){
