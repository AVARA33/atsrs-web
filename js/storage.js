...
restoreCurrentPage=function(){let params=new URLSearchParams(location.search),route=params.get('route'),developerRequested=route==='developer',page=(route==='profile'||route==='dashboard')?route:(developerRequested?'dashboard':(localStorage.getItem('atsrs_current_page')||'intro'));window.__atsrsRequestedDeveloperRoute=developerRequested;let map={intro:navIntro,privacy:navPrivacy,dataRights:navPrivacy,dashboard:navDashboard,candidates:navCandidates,personnel:navPersonnel,certificates:navCertificates,refs:navRefs,compliance:navCompliance,security:navCompliance,reports:navReports,profile:navProfile,jobs:navJobs,employers:navEmployers,recruiters:navRecruiters};showPage(map[page]?page:'intro',map[page]||navIntro);if(developerRequested){let routeUrl=new URL(location.href);routeUrl.searchParams.set('route','developer');history.replaceState({},'',routeUrl.pathname+routeUrl.search+routeUrl.hash);}if(page==='profile'&&params.get('tab')==='sharing')setTimeout(function(){showAccountTab('sharing')},0);}
const V27_TEXT={
