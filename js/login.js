...
/* ===== ATSRS V199 Classic Google OAuth Workspace Flow ===== */
(function(){
  'use strict';
  var APP_HOME = 'https://atsrs.com';
  function byId(id){return document.getElementById(id);}
  function msg(text){var el=byId('loginMsg'); if(el) el.textContent=text||'';}
  function safeEmail(user){return (user && (user.email || (user.user_metadata&&user.user_metadata.email))) || '';}
  function baseUserId(user){return (user && user.id) || safeEmail(user) || 'google_user';}
  function wsKey(user){return 'atsrs_google_workspaces_' + (safeEmail(user)||baseUserId(user)).toLowerCase();}
  function getWorkspaces(user){try{return JSON.parse(localStorage.getItem(wsKey(user))||'[]').filter(Boolean);}catch(e){return [];}}
  function setWorkspaces(user, arr){try{localStorage.setItem(wsKey(user), JSON.stringify(Array.from(new Set(arr))));}catch(e){}}
  function label(mode){return mode==='company'?'Corporate':'Personal';}
  function setModeAndOpen(user, mode){
    var cleanMode = mode==='company' ? 'company' : 'personal';
    var workspaces = getWorkspaces(user);
    if(workspaces.indexOf(cleanMode)<0){ workspaces.push(cleanMode); setWorkspaces(user, workspaces); }
    try{localStorage.setItem('atsrs_use_mode', cleanMode);}catch(e){}
    try{window.useMode=cleanMode;}catch(e){}
    var u = Object.assign({}, user || {});
    u.id = baseUserId(user) + '_' + cleanMode;
    u.email = safeEmail(user);
    try{window.currentUser=u; currentUser=u;}catch(e){window.currentUser=u;}
    if(typeof window.openApp==='function') window.openApp();
  }
  function ensureModal(){
    var modal=byId('atsrsWorkspaceModal');
    if(modal) return modal;
    modal=document.createElement('div');
    modal.id='atsrsWorkspaceModal';
    modal.className='atsrs-workspace-modal hidden';
    modal.innerHTML='<div class="atsrs-workspace-card"><h3 id="atsrsWorkspaceTitle">Choose workspace</h3><p id="atsrsWorkspaceText" class="sub">Select how you want to continue.</p><div id="atsrsWorkspaceActions" class="atsrs-workspace-actions"></div></div>';
    document.body.appendChild(modal);
    return modal;
  }
  function showWorkspaceChoice(user, modeList, text){
    var modal=ensureModal();
    var title=byId('atsrsWorkspaceTitle'), body=byId('atsrsWorkspaceText'), actions=byId('atsrsWorkspaceActions');
    if(title) title.textContent='Choose workspace';
    if(body) body.textContent=text || 'Select how you want to continue.';
    if(actions){
      actions.innerHTML='';
      modeList.forEach(function(mode){
        ...
        b.type='button';
        b.className='primary';
        b.textContent=(mode==='company'?'Corporate Account':'Personal Account');
        b.onclick=function(){modal.classList.add('hidden'); setModeAndOpen(user, mode);};
  ...
  function handleSession(session){
    ...
    var flow='signin';
    try{flow=localStorage.getItem('atsrs_google_flow')||'signin';}catch(e){}
    var workspaces=getWorkspaces(user);
    if(flow==='signup'){
      var available=['personal','company'].filter(function(m){return workspaces.indexOf(m)<0;});
      if(!available.length) available=['personal','company'];
      showWorkspaceChoice(user, available, 'Create a Personal or Corporate workspace for '+safeEmail(user)+'.');
      try{localStorage.removeItem('atsrs_google_flow');}catch(e){}
      return;
    }
    if(workspaces.length===1){ setModeAndOpen(user, workspaces[0]); return; }
    if(workspaces.length>1){ showWorkspaceChoice(user, workspaces, 'Continue as Personal or Corporate.'); return; }
    showWorkspaceChoice(user, ['personal','company'], 'No workspace found for this Google account. Create one to continue.');
  }
  async function startGoogle(flow){
    msg('');
    if(!window.supabaseClient || !window.supabaseClient.auth){ msg('Supabase Auth is not loaded.'); return; }
    try{localStorage.setItem('atsrs_google_flow', flow || 'signin');}catch(e){}
    var redirectTo = (location.hostname==='localhost' || location.hostname==='127.0.0.1') ? location.origin + location.pathname : APP_HOME;
    var res = await window.supabaseClient.auth.signInWithOAuth({provider:'google', options:{redirectTo:redirectTo}});
    if(res && res.error) msg(res.error.message || 'Google sign-in failed.');
  }
  function bind(){
    var signIn=byId('googleSigninBtn');
    var signUp=byId('googleSignupBtn');
    if(signIn) signIn.onclick=function(e){if(e)e.preventDefault(); startGoogle('signin');};
    if(signUp) signUp.onclick=function(e){if(e)e.preventDefault(); startGoogle('signup');};
    ['createBtn','forgotBtn','loginEmail','loginPassword','modeChoiceBox','modeRule','modeInstruction'].forEach(function(id){var el=byId(id); if(el) el.style.display='none';});
    if(window.atsrsSupabaseSession) handleSession(window.atsrsSupabaseSession);
    else if(window.supabaseClient && window.supabaseClient.auth){
      window.supabaseClient.auth.getSession().then(function(res){var s=res&&res.data&&res.data.session; if(s) handleSession(s);});
    }
  }
  window.atsrsHandleOAuthSession=handleSession;
  window.atsrsStartGoogleAuth=startGoogle;
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', bind); else bind();
})();
