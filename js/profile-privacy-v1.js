(function(){
  ...
  var FIELD_DEFINITIONS=[
    ...
    {key:'salary',icon:'ph-currency-dollar',title:'Expected salary',note:'Display your expected salary range'},
  ...
  function applyModeEffects(){var mode=draft&&draft.mode||'custom',matrix=byId('profilePrivacyRows');if(matrix)matrix.classList.toggle('is-locked',mode!=='custom');document.querySelectorAll('.profile-privacy-mode-card').forEach(function(card){var input=card.querySelector('input');card.classList.toggle('is-selected',!!input&&input.checked)});var audience=byId('profilePrivacyAudience');if(audience){if(mode==='public')audience.value='everyone';else if(mode==='private')audience.value='only_me';else audience.value=draft.audience;audience.disabled=mode!=='custom'}}
  function render(settings){draft=clone(settings);document.querySelectorAll('input[name="profilePrivacyMode"]').forEach(function(input){input.checked=input.value===draft.mode});var audience=byId('profilePrivacyAudience');if(audience)audience.value=draft.audience;Object.keys(draft.fields).forEach(function(key){var input=document.querySelector('input[name="privacy_'+key+'"][value="'+draft.fields[key]+'"]');if(input)input.checked=true});applyModeEffects()}
  function readDraft(){if(!draft)return;var mode=document.querySelector('input[name="profilePrivacyMode"]:checked'),audience=byId('profilePrivacyAudience');draft.mode=normalizeMode(mode&&mode.value);draft.audience=normalizeAudience(audience&&audience.value);Object.keys(DEFAULT_FIELDS).forEach(function(key){var input=document.querySelector('input[name="privacy_'+key+'"]:checked');draft.fields[key]=input?input.value:DEFAULT_FIELDS[key]});applyModeEffects()}
  ...
  function bind(){renderRows();document.querySelectorAll('input[name="profilePrivacyMode"]').forEach(function(input){input.addEventListener('change',function(){readDraft();if(input.value==='custom'&&draft)draft.audience='recruiters';applyModeEffects();scheduleSave()})});var audience=byId('profilePrivacyAudience');if(audience)audience.addEventListener('change',function(){readDraft();draft.mode=audience.value==='everyone'?'public':audience.value==='only_me'?'private':'custom';render(draft);scheduleSave()});render(fromProfile(readProfile()));window.addEventListener('atsrs:cloud-data-loaded',function(){render(fromProfile(readProfile()))})}
