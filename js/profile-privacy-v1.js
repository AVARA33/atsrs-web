(function(){
  ...
  function syncFieldsForMode(){if(!draft||draft.mode==='custom')return;Object.keys(DEFAULT_FIELDS).forEach(function(key){draft.fields[key]=draft.mode;var input=document.querySelector('input[name="privacy_'+key+'"][value="'+draft.mode+'"]');if(input)input.checked=true})}
  ...
  function render(settings){draft=clone(settings);document.querySelectorAll('input[name="profilePrivacyMode"]').forEach(function(input){input.checked=input.value===draft.mode});syncFieldsForMode();Object.keys(draft.fields).forEach(function(key){var input=document.querySelector('input[name="privacy_'+key+'"][value="'+draft.fields[key]+'"]');if(input)input.checked=true});applyModeEffects()}
  function readDraft(){if(!draft)return;var mode=document.querySelector('input[name="profilePrivacyMode"]:checked');draft.mode=normalizeMode(mode&&mode.value);draft.audience=draft.mode==='public'?'everyone':draft.mode==='private'?'only_me':'recruiters';if(draft.mode==='custom')Object.keys(DEFAULT_FIELDS).forEach(function(key){var input=document.querySelector('input[name="privacy_'+key+'"]:checked');draft.fields[key]=input?input.value:DEFAULT_FIELDS[key]});else syncFieldsForMode();applyModeEffects()}
