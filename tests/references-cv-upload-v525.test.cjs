...
assert.match(index, /class="cv-generator-action-grid">[\s\S]*?id="generateCVBtn"[\s\S]*?id="uploadCvFromGeneratorBtn"[^>]*onclick="replaceCV\(\)"[^>]*>Enhance Existing CV<\/button>[\s\S]*?id="generatedCvActions"[\s\S]*?id="previewGeneratedCvBtn"[\s\S]*?id="printGeneratedCvBtn"/);
