...
vm.runInContext('var NEW_MS=86400000;'+source.slice(start,end),context);
...
assert.equal(context.isNew(job,at(23*60*60*1000+59*60*1000+59*1000)),true,'23:59:59 remains NEW');
assert.equal(context.isNew(job,at(24*60*60*1000)),false,'24:00:00 expires');
...
assert.match(source,/setAttribute\('aria-label','New vacancy published within the last 24 hours'\)/);
