...
assert.equal(version, 'V388');
assert.equal(update, '29 Jul 2026');

for (const asset of ['stability-runtime.js', 'server-data.js', 'storage.js', 'workspace-switcher.js']) {
  assert.match(index, new RegExp(`src="js/${asset.replace('.', '\\.')}\\?v=388"`));
}
...
assert.match(runbook, /Frontend release: V388/);

console.log('V388 build marker consistency tests passed');
