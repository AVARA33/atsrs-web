...
async function check({url='https://atsrs.com/s/abcdefghijklmnopqrstuv',valid=true,clipboard=true}={}){
  const calls={ids:[],copied:[],messages:[],cleared:[]};
  const context={window:{alert(){},dispatchEvent(){}},URL,CustomEvent:class{},knownShareUrl:'',
    ensureShortShareLink:async id=>{calls.ids.push(id);return url},validateShareUrl:async()=>valid,
    copyShareReference:async value=>{calls.copied.push(value);return clipboard},
    ownerMessage:(text,error)=>calls.messages.push({text,error}),
    setOwnerToken:(...args)=>calls.cleared.push(args),setOwnerShortCode:(...args)=>calls.cleared.push(args),setKnownLink(){},byId:()=>null,setTimeout,friendlyError:(_e,f)=>f,uiText:value=>value};
  vm.runInNewContext(copy,context);
  calls.result=await context.window.copyShareLink('existing-share-id');return calls;
}
(async()=>{
  let result=await check();assert.equal(result.result,true);assert.deepEqual(result.ids,['existing-share-id']);assert.deepEqual(result.copied,['https://atsrs.com/s/abcdefghijklmnopqrstuv']);
