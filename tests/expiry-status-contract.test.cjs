...
assert.equal(canonical.rows[0].dashboard_expiring_60_count, 1, 'Dashboard contract must place the document in the 60-day window');
assert.equal(canonical.rows[0].dashboard_expiring_30_count, 0);
assert.equal(canonical.rows[0].dashboard_expiring_7_count, 0);
...
assert.match(reportingSource, /\['Expiring in 90 days',row\.dashboard_expiring_90_count\]/);
assert.match(reportingSource, /\['Expiring in 60 days',row\.dashboard_expiring_60_count\]/);
assert.match(reportingSource, /\['Expiring in 30 days',row\.dashboard_expiring_30_count\]/);
assert.match(reportingSource, /\['Expiring in 1 week',row\.dashboard_expiring_7_count\]/);
