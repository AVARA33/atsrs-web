...
const source = fs.readFileSync(path.join(__dirname, '..', 'js', 'workspace-switcher.js'), 'utf8');
...
function boot(switchWorkspace) {
  ...
  vm.runInNewContext(source, { window, document, localStorage, Promise, console }, {
    filename: 'workspace-switcher.js'
  });
  return { window, localStorage, elements };
}

async function testThirtySequentialSwitchesAndLoaderCompletion() {
  let calls = 0;
...
async function testParallelAttemptIsSingleFlightAndRenderCannotUnlockControls() {
  ...
  assert.equal(await conflicting, true);
  assert.equal(app.elements.workspaceSwitcher.getAttribute('aria-busy'), 'false');
}
...
function testHydrationGateAndVerificationRefreshAreReadOnly() {
  ...
  assert.match(dashboard, /verificationRefreshPromise/);
  assert.match(dashboard, /Promise\.all\(kinds\.map/);
  const refresh = dashboard.slice(
    dashboard.indexOf('async function refreshVerificationStatus()'),
    dashboard.indexOf('function bindOfficialProfileControls()')
  );
  assert.doesNotMatch(refresh, /writeJson\(/);
  assert.match(dashboard, /function profileHydrationPending\(\)/);
  assert.match(dashboard, /if\(profileHydrationPending\(\)\)return false/);
  assert.match(dashboard, /addEventListener\('atsrs:data-hydrated'/);
}
