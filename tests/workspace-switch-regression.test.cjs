...
async function testTwentySequentialSwitchesAndLoaderCompletion() {
  let calls = 0;
...
function testStorageReloadRollbackAndSequenceContracts() {
  ...
  assert.match(storage, /sequence!==workspaceSwitchSequence/);
  assert.match(storage, /applyAccountType\(current\)/);
  assert.match(storage, /window\.location\.reload\(\)/);
}

function testVerificationWritesAreCoalescedAndDeduplicated() {
  ...
  assert.match(dashboard, /if\(profile\[field\]!==result\.verified\)/);
  assert.match(dashboard, /if\(changed\)writeJson\(PROFILE_KEY,profile\)/);
}

(async () => {
  await testTwentySequentialSwitchesAndLoaderCompletion();
  ...
  testVerificationWritesAreCoalescedAndDeduplicated();
