...
assert.match(index,/css\/shell-polish\.css\?v=467/);
...
assert.match(css,/Personal and Corporate routes share the same readable content origin[\s\S]*?body:where\(\.personal-mode,\.company-mode\)[\s\S]*?#pageTitle,[\s\S]*?> \.main > section[\s\S]*?width:min\(100%,1440px\)/);
assert.match(css,/Corporate routes already carry their own contextual heading[\s\S]*?body\.company-mode:has\(#dashboardPage:not\(\.hidden\)\)[\s\S]*?#pageTitle[\s\S]*?display:none!important/);
assert.match(css,/body\.company-mode:has\(#privacyPage:not\(\.hidden\)\)[\s\S]*?> \.main > #pageTitle/);
assert.doesNotMatch(css,/body\.company-mode:has\(#(?:candidatesPage|personnelPage):not\(\.hidden\)\)[^\{]*#pageTitle\{[\s\S]*?display:none!important/);
