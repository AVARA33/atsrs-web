...
test('Candidate directory is a bounded service-only projection', () => {
  const migration = read('supabase/migrations/20260816184500_bounded_talent_directory_page.sql');
  ...
  assert.match(migration, /exists\s*\([\s\S]*from public\.atsrs_files/i);
...
test('Candidate client uses keyset cursor and exposes explicit continuation', () => {
  ...
  assert.match(html, /js\/talent-directory\.js\?v=570/);
});
