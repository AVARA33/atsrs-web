...
assert.match(index, /css\/pricing\.css\?v=5996/);
assert.match(pricing, /css\/pricing\.css\?v=5996/);
...
assert.match(pricing, /1 lifetime AI document scan \+ 1 lifetime AI CV generation/);
assert.match(pricing, /<th scope="row">AI Document Scans<\/th><td>1 lifetime<\/td><td>15 \/ month<\/td>/);
...
assert.match(pricing, /data-monthly="\$19\.99" data-yearly="\$16"[\s\S]*?data-yearly-note="Save 20% · \$192 billed annually"/);
assert.match(pricing, /data-monthly="\$39\.99" data-yearly="\$32"[\s\S]*?data-yearly-note="Save 20% · \$384 billed annually"/);
assert.match(pricing, /data-monthly="\$69\.99" data-yearly="\$56"[\s\S]*?data-yearly-note="Save 20% · \$672 billed annually"/);
assert.match(pricing, /id="titan"[\s\S]*?data-monthly="\$119\.99" data-yearly="\$96"[\s\S]*?data-yearly-note="Save 20% · \$1,152 billed annually"/);
...
assert.match(pricing, /Additional capacity is available separately for each product/);
assert.match(pricing, /Increase only the allowance you need—Tracked Documents, Storage, AI Document Scans, AI CV Generations or WhatsApp Expiry Alerts—without changing your plan/);
...
assert.match(pricing, /js\/pricing\.js\?v=467/);
assert.match(runtime, /cycleStorageKey='atsrs_pricing_cycle'/);
assert.match(runtime, /localStorage\.setItem\(cycleStorageKey,cycle\)/);
assert.match(runtime, /applyCycle\(storedCycle\(\)\)/);
