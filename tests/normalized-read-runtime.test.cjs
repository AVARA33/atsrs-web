...
(async () => {
  ...
  const shadowScript = index.indexOf('js/shadow-read.js?v=405');
  const adapterScript = index.indexOf('js/normalized-read-adapter.js?v=405');
  const configScript = index.indexOf('js/normalized-read-canary-config.js?v=405');
  const runtimeScript = index.indexOf('js/normalized-read-runtime.js?v=405');
  const storageScript = index.indexOf('js/storage.js?v=405');
  assert.ok(
