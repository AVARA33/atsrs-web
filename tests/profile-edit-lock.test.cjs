...
assert.match(index,/js\/dashboard\.js\?v=5923/);
assert.match(index,/js\/profile-workspace-v5886\.js\?v=5928/);
...
assert.match(dashboard,/if\(!personalWorkspace&&editButton&&!editButton\.__atsrsProfileEditBound\)[\s\S]*?editButton\.addEventListener\('click',function\(\)\{setProfileEditMode\(true,true\)\}\)/);
...
assert.match(dashboard,/if\(!personalWorkspace&&generalTab&&!generalTab\.hasAttribute\('data-profile-editing'\)\)setProfileEditMode\(false,false\)/);
assert.match(dashboard,/updateProfileSummary\(data\);\s*updateProfileStage\(data\);\s*if\(!document\.body\.classList\.contains\('personal-mode'\)\)setProfileEditMode\(false,false\);\s*showSaved\(\);return true/);
