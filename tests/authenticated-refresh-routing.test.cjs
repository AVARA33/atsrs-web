...
assert.match(index, /data-atsrs-build="V505"/);
...
assert.match(storage, /ATSRS passive workspace restore failed[\s\S]*?returnToLogin\(user,workspaceServiceMessage\(error\)\);[\s\S]*?return false/);
