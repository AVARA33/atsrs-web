...
test('Jobs is isolated, navigable and visibly live',()=>{
  ...
  assert.match(index,/jobs-prototype\.css\?v=6146/);
  assert.match(index,/route-feature-loader\.js\?v=6146/);
  assert.doesNotMatch(index,/<script src="js\/jobs-prototype\.js\?v=58163"><\/script>/);
  assert.match(routeLoader,/loadScript\('js\/jobs-prototype\.js\?v=6146'\)/);
...
test('Jobs is fixed to the standard card view',()=>{
  ...
  assert.match(runtime,/if\(linkedinHref\)contact\(actions,'Recruiter LinkedIn','Open LinkedIn','linkedin',linkedinHref\)/);
  ...
  assert.match(runtime,/Math\.max\(max,Math\.ceil\(card\.getBoundingClientRect\(\)\.height\)\)/);
  assert.match(runtime,/slot\.style\.setProperty\('--jobs-card-collapsed-height',tallest\+'px'\)/);
  assert.match(runtime,/window\.addEventListener\('resize',scheduleCardHeights\)/);
  assert.match(css,/\.jobs-grid\.jobs-cards\{align-items:stretch\}/);
  assert.match(css,/\.jobs-cards \.job-card\{height:100%;[^}]*grid-template-rows:auto minmax\(0,1fr\) auto;align-content:stretch\}/);
  assert.match(css,/\.job-card-slot\{[^}]*height:100%/);
  assert.match(css,/\.job-card-slot\.is-disclosure-open>\.job-card\{[^}]*height:auto;min-height:100%/);
  assert.match(css,/\.jobs-cards \.job-card-slot\{height:var\(--jobs-card-collapsed-height,auto\)\}/);
  assert.match(css,/\.jobs-cards \.job-card-slot>\.job-card\{height:var\(--jobs-card-collapsed-height,auto\)/);
  assert.match(css,/\.jobs-cards \.job-card-body\{[^}]*min-height:0[^}]*overflow:visible/);
  ...
  assert.match(css,/@media\(max-width:600px\)\{\.jobs-cards \.job-card\{height:100%;min-height:0;max-height:none\}/);
  ...
  assert.match(css,/\.jobs-cards \.job-contact-info\{align-content:start\}/);
  ...
  assert.doesNotMatch(runtime,/function atsrsJobUrl\(job\)/);
