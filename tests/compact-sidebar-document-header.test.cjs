...
assert.match(index, /js\/sage-ledger\.js\?v=450/, 'V450 navigation repair runtime must bypass the production cache');
assert.match(runtime, /setNav\(byId\('navIntro'\),'Product Updates','sparkle'\)/, 'Product Updates must retain its Phosphor icon');
assert.match(runtime, /new MutationObserver\(function\(\)\{[\s\S]*?updates&&!updates\.querySelector\('\.sage-nav-icon'\)\)decorateNavigation\(\);[\s\S]*?\}\)\.observe\(navigation,\{childList:true,subtree:true,characterData:true\}\)/, 'Navigation observer must repair legacy text-only rewrites');
...
assert.match(harness, /js\/sage-ledger\.js\?v=450/, 'Visual harness must load the navigation repair runtime');
assert.match(harness, /navIntro'\)\.textContent='ATSRS Updates'/, 'Visual harness must reproduce the delayed legacy rewrite');
