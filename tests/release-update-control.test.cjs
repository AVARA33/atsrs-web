...
async function run({ current = false, persisted = false, attempted = '' } = {}) {
  ...
  const document = { documentElement: { getAttribute() { return current ? 'V6156' : 'V6155'; } } };
  ...
  const context = {
    ...
    fetch: async () => ({ ok: true, text: async () => '<html data-atsrs-build="V6156"></html>' })
  };
...
(async () => {
  ...
  assert.match(stale.replaced[0], /_atsrs_release=V6156-/);

  const current = await run({ current: true, attempted: 'V6156' });
  ...
  assert.equal((await run({ attempted: 'V6156' })).replaced.length, 0, 'the entry reload guard prevents a loop');
