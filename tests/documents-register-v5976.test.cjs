...
assert.match(html, /data-atsrs-build="V6094"/);
...
assert.match(html, /css\/documents-register-v5976\.css\?v=6135/);
assert.match(html, /js\/app\.js\?v=6126/);
assert.match(html, /js\/server-data\.js\?v=6018/);
assert.match(app, /className='atsrs-document-folder-tab'/);
assert.match(app, /function toggleDocumentFolderPin\(id\)/);
assert.doesNotMatch(app, /className='atsrs-document-folder-pin'/);
assert.match(app, /className='atsrs-document-folder-rename-pin'/);
assert.match(app, /ph-floppy-disk/);
assert.match(app, /function documentFolderIcon\(label\)/);
assert.match(app, /return 'ph-bank'/);
assert.match(app, /return 'ph-car'/);
assert.match(app, /return 'ph-first-aid-kit'/);
assert.match(app, /folder\.pinned=!folder\.pinned/);
assert.match(app, /dragged\.pinned\|\|!targetFolder\|\|targetFolder\.pinned/);
assert.match(app, /wrap\.draggable=!\(folder&&folder\.pinned\)/);
assert.match(app, /Number\(!!b\.pinned\)-Number\(!!a\.pinned\)/);
assert.match(app, /seen\.has\(String\(folder\.id\)\)/);
assert.match(serverData, /_documentFolders\$/);
assert.match(serverData, /return 'folder:'\+String\(item\.id\)/);
assert.doesNotMatch(css, /#certificatesPage \.atsrs-document-folder-pin/);
...
assert.match(app, /data-cert-folder-trigger/);
assert.match(app, /tab\('all','All',certs\.length\)/);
assert.match(app, /function openDocumentFolderMoveMenu\(trigger,index\)/);
assert.doesNotMatch(app, /ph-folder-notch-open/);
assert.match(app, /if\(folderId==='all'\)delete certs\[index\]\.folderId/);
assert.doesNotMatch(app, /tab\('unfiled'/);
