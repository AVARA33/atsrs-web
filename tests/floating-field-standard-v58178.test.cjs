...
assert.match(index, /data-atsrs-build="V5834"/);
assert.match(index, /floating-field-standard-v58178\.css\?v=58193/);
...
assert.match(css, /html\[data-theme="dark"\] body #app\.app #jobsPage \.atsrs-field-shell\{[\s\S]*border-color:var\(--atsrs-field-accent\)!important;[\s\S]*box-shadow:0 0 0 3px var\(--atsrs-field-accent-ring\),0 8px 20px var\(--atsrs-field-accent-shadow\)!important/);
assert.match(css, /html\[data-theme="dark"\] body #app\.app #jobsPage \.atsrs-field-shell>\.atsrs-field-label\{[\s\S]*color:var\(--atsrs-field-accent\)!important/);
...
console.log('V5834 Jobs filter persistent Search-ring contracts passed');
