...
function harness() {
  const ids=['aiCvUploadInput','aiCvSourceStatus','cvEnhancementConsent','cvEnhancementConsentWrap','cvEnhancementStatus','cvGeneratorStatus','cvBetaBadge','cvBetaTitle','cvBetaText','uploadCvFromGeneratorBtn','generateCVBtn','resetCvGeneratorBtn','generatedCvActions','previewGeneratedCvBtn','downloadGeneratedCvBtn','printGeneratedCvBtn','addGeneratedCvBtn','replaceMainWithGeneratedCvBtn','regenerateCvBtn','savePdfCvBtn','closeCvGeneratorBtn','cvGeneratorModal','cvGeneratorPreview','cvGeneratorPreviewDocument','cvStatusBadge','cvFileInfo'];
  ...
  const context={console:{error(){}},alert(){},confirm(){return allowReplace;},localStorage:{getItem(k){return k==='atsrs_use_mode'?'personal':null;}},File,Blob,URL,AbortController,AbortSignal,Date,JSON,Object,Array,String,Promise,
    ...
    window:{useMode:'personal',localKey(){return'profile';},print(){},supabaseClient:{auth:{async getSession(){return{data:{session:{access_token:'token'}}};}},functions:{async invoke(_name,options){requests.push(options.body);return{data:{cv:{full_name:'Generated',headline:'Role',contact:{},professional_summary:'Fresh',core_skills:[],experience:[],education:[],certifications:[]},variation_index:requests.length-1},error:null};}}},atsrsCloudData:{isManagedKey(){return false;},async uploadAiCvSource(file){temporary.push(['upload',file.name]);return'user/personal/ai-cv-source/source.pdf';},async deleteAiCvSource(path){temporary.push(['delete',path]);},async saveGeneratedCv(file,options){saved.push({name:file.name,replaceMain:options.replaceMain});},async renderFiles(){}}}
  };
  ...
  return {elements,requests,saved,temporary,api:context.window.atsrsCvGenerator,setConfirm(v){allowReplace=v;}};
}
...
(async()=>{
  ...
  assert.equal(t.saved.length,0,'generation must not auto-save');
  ...
  t.elements.cvEnhancementConsent.checked=true;
  await t.elements.generateCVBtn.dispatch('click');
  await t.elements.addGeneratedCvBtn.dispatch('click');
  assert.equal(t.saved.at(-1).replaceMain,false);
  t.setConfirm(false);
  await t.elements.replaceMainWithGeneratedCvBtn.dispatch('click');
  assert.equal(t.saved.length,1,'cancelled confirmation must not mutate Profile');
  t.setConfirm(true);
  await t.elements.replaceMainWithGeneratedCvBtn.dispatch('click');
  assert.equal(t.saved.at(-1).replaceMain,true);
