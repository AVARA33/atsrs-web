...
assert.match(html,/css\/profile-privacy-v1\.css\?v=27/);
...
assert.match(css,/profile-privacy-radio input\{[^}]*accent-color:var\(--profile-privacy-choice-accent\)/);
...
assert.match(css,/data-theme[^}]*profile-privacy-mode-card\.is-selected[^}]*\{[^}]*border-color:var\(--atsrs-field-focus-block-line\) var\(--atsrs-field-focus-inline-line\)!important/);
assert.match(css,/profile-privacy-mode-card\.is-selected :is\(strong,\.profile-privacy-mode-icon\)\{color:var\(--atsrs-field-accent\)!important/);
...
assert.match(html,/js\/profile-privacy-v1\.js\?v=15/);
assert.match(js,/hasSaved&&legacyMode&&visibilityValue\(savedMode\)!==legacy\?legacyMode:savedMode/);
...
assert.match(js,/if\(edit\)edit\.hidden=false/);
assert.match(js,/input\.dataset\.privacyValue=value/);
assert.match(js,/data-privacy-value/);
assert.match(js,/input\.disabled=false;input\.tabIndex=rowsLocked\?-1:0/);
assert.match(js,/PRIVACY_MODE_STORAGE_KEY='atsrs_profile_privacy_mode'/);
assert.match(js,/rememberedMode\(\)\|\|legacyMode\|\|'custom'/);
assert.doesNotMatch(js,/legacy&&\/\^\(Public\|Private\|Link Only\)\$\//);
