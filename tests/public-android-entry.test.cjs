...
assert.match(index, /href="contact\.html">Contact<\/a>\s*<\/nav>/);
assert.doesNotMatch(index, /public-android-entry/);
assert.doesNotMatch(index, /ATSRS for Android coming soon/);
...
assert.match(css, /\.public-header-actions\{[^}]*min-width:0[^}]*flex-wrap:nowrap/);
assert.match(css, /@media\(max-width:1350px\) and \(min-width:721px\)[\s\S]*?\.public-nav\{display:none\}[\s\S]*?\.public-home-mobile\{display:inline-flex\}/);
...
assert.doesNotMatch(index, /href="[^"]+\.apk(?:["?#]|$)/i);

console.log('Public header omits the Android coming-soon entry');
