...
assert.match(index, /data-atsrs-build="V5920"/);
assert.match(index, /href="css\/jobs-prototype\.css\?v=5920"/);
...
assert.match(css, /background:color-mix\(in srgb,var\(--atsrs-field-accent\) 5%,var\(--atsrs-field-surface\)\)/);
...
console.log('V5920 JobSearch contact layout and readability release is cache-busted');
