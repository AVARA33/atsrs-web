...
assert.match(runtime, /description&&norm\(description\)!==norm\(summary\)/);
assert.match(runtime, /el\('section','job-card-description'\)/);
assert.match(runtime, /el\('p','',description\)/);
...
console.log('Job cards show a readable excerpt of the stored source description.');
