...
assert.match(runtime, /backdrop\.onclick=function\(\)\{document\.querySelectorAll\('\.job-card-slot\.is-disclosure-open'\)\.forEach\(closeCardDisclosures\);removeCardDisclosureBackdrop\(\)\}/);
...
assert.match(css, /\.job-card-disclosure-backdrop\{display:none;position:fixed;z-index:70;inset:0;[^}]*backdrop-filter:blur\(4px\)/);
assert.match(css, /#jobsPage\.has-job-card-disclosure \.job-card-disclosure-backdrop\{display:block\}/);
