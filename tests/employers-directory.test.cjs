...
assert.match(html, /css\/employers\.css\?v=29/);
...
assert.match(css, /html\[data-theme\] body #app\.app:not\(\.hidden\) #employersPage \.employer-actions \.employer-action-jobs \{[\s\S]*?border-color: transparent !important;[\s\S]*?background: transparent !important;[\s\S]*?box-shadow: none !important;/);
