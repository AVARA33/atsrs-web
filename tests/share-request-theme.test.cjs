...
assert.doesNotMatch(css, /html\[data-theme="dark"\][^{]*\.share-request-dialog input:focus[^}]*#36c85b/, 'the legacy green request focus must be removed');
assert.match(index, /css\/share-profile\.css\?v=511/, 'the browser must receive the refreshed request dialog theme');
