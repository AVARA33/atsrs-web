...
assert.match(source,/contact\(actions,'Recruiter email',validEmail\(job\.recruiter_email\),'email',mailtoHref\(job\)\)/);
assert.doesNotMatch(source,/Send email/);
assert.doesNotMatch(source,/function action\(/);
assert.doesNotMatch(source,/['"]tel:/);
assert.match(source,/applicationUrl\(job\.application_url\)\|\|mailtoHref\(job\)/);
