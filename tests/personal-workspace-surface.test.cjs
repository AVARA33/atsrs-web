...
assert.match(css, /@media\(max-width:720px\)[\s\S]*?:is\([\s\S]*?#profilePage[\s\S]*?\) :where\(button,\[role="button"\],summary\)[\s\S]*?min-height:44px!important/);
assert.doesNotMatch(css, /body\.company-mode/);
assert.doesNotMatch(css, /linear-gradient|radial-gradient/);

assert.match(index, /href="css\/personal-workspace-surface\.css\?v=425"/);
assert.match(index, /data-atsrs-build="V425"/);
