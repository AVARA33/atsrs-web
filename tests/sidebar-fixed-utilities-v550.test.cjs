...
assert.match(html, /data-atsrs-build="V564"/);
...
assert.match(css, /body\.personal-mode\.atsrs-personal-dashboard-route[\s\S]*?> \.sidebar\{[\s\S]*?position:fixed!important;[\s\S]*?height:100svh!important/);
assert.match(css, /body\.personal-mode\.atsrs-personal-dashboard-route[\s\S]*?> \.sidebar\.v76-mobile-closed > \.nav\{[\s\S]*?display:grid!important/);
...
assert.match(harness, /personal-dashboard-qa\.css\?v=450/);
assert.match(harness, /sidebar-fixed-utilities-v550\.css\?v=561/);
assert.match(harness, /shell-polish\.js\?v=555/);
assert.match(harness, /personal-mode atsrs-personal-dashboard-route/);
assert.match(harness, /sidebar v76-mobile-closed/);
assert.match(harness, /id="navIntro"[\s\S]*?id="navPrivacy"/);

console.log('V564 fixed sidebar and Dashboard mobile contracts passed');
