...
assert.match(html, /css\/sidebar-fixed-utilities-v550\.css\?v=552/);
...
assert.match(css, /display:flex!important;[\s\S]*?flex-direction:column!important;[\s\S]*?height:100dvh!important/);
...
assert.match(harness, /sidebar-fixed-utilities-v550\.css\?v=552/);
assert.match(harness, /id="navIntro"[\s\S]*?id="navPrivacy"/);

console.log('V552 fixed sidebar utility navigation contracts passed');
